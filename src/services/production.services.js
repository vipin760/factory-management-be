...
exports.getAllProductionService1 = async (query) => {
  try {
    const {
      search,            // keyword search for batch_no or article_sku
      status,            // filter by status
      start_date,        // filter production start date (from)
      end_date,          // filter production end date (to)
    ...
    baseQuery += `
      GROUP BY pb.id
      ORDER BY pb.${sort_by} ${order === 'asc' ? 'ASC' : 'DESC'}
 ...
 const response = {result,total,page,limit }
    return {
      status: true,
      data:response,
...
exports.getAllProductionService2 = async (query) => {
  try {
    const {
      search,
      status,
    ...
    let baseQuery = `
      SELECT pb.*, 
        ...
        ) FILTER (WHERE bc.id IS NOT NULL), '[]') AS batch_consumptions
      FROM production_batches pb
      LEFT JOIN batch_consumptions bc ON bc.production_batch_id = pb.id
      LEFT JOIN raw_material_batches rmb ON rmb.id = bc.raw_material_batch_id
      LEFT JOIN raw_materials rm ON rm.id = rmb.raw_material_id
    ...
    baseQuery += ` GROUP BY pb.id ORDER BY pb.${sort_by} ${order === 'asc' ? 'ASC' : 'DESC'}`;
    ...
    // Total count for pagination (always useful)
    let countQuery = `SELECT COUNT(*) AS total FROM production_batches pb WHERE 1=1`;
    const countValues = [];
    let countIdx = 1;
...
exports.getAllProductionService3 = async (query) => {
  try {
    const {
      search,
      status,
    ...
    // Base query with expenditure + materials details
    let baseQuery = `
      WITH batch_data AS (
        SELECT 
          pb.id AS batch_id,
    ...
    return {
      ...
      message: "Production batches with expenditure fetched successfully."
    };
...
exports.getAllProductionService4 = async (query) => {
  try {
    const {
      search,
      status,
    ...
    // Base query with batch + consumptions + material cost
    let baseQuery = `
      WITH batch_data AS (
        SELECT 
          pb.id AS batch_id,
    ...
    return {
      ...
      message: "Production batches with material cost & consumptions fetched successfully."
    };
