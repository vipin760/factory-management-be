...
exports.getCustomerOrdersById = catchAsync(async (req, res, next) => {
    const { status, message, data } = await customerOrderServices.getCustomerOrderById(req.params.id);
    if (!status) return next(new ErrorHandler(message, 400));
    return res.status(200).send({ status, data, message })
})

exports.transferOrReturnCustomerOrderService = catchAsync(async (req, res, next) => {
    const { status, message, data } = await customerOrderServices.transferOrReturnCustomerOrder(req.body,req.user.id);
    if (!status) return next(new ErrorHandler(message, 400));
    return res.status(200).send({ status, data, message })
})
