...
exports.getAllIndentwisePurchaseHistoryController = catchAsync( async(req ,res ,next)=>{
    const {status, data, message} =await historyServices.getAllIndentwisePurchaseHistoryService(req.query)
    if(!status) return next(new ErrorHandler(message,400));
    return res.status(200).send({status,data,message})
})